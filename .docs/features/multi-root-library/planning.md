...
| 2026-09-16 | Added `GET /library/browse` (admin-only, read-only directory listing) and `LibraryBrowseDialog` on Options, so adding a root no longer requires already knowing its exact path. Paired with a `docker-compose.yml` change mounting every Windows drive at once, read-only (`/mnt:/mnt:ro`) — WSL2 already exposes them there, so a new drive needs no further compose edit or restart to become browsable. 291/291 backend tests (6 new in `routes/library.test.ts`). Verified with Playwright against the rebuilt container, including browsing into `/root` (genuinely unreadable by the container's `node` user) to confirm the dialog shows a clean error rather than crashing. | A native OS folder picker can't do this — it never exposes a picked folder's real path, and the relevant filesystem is the container's, not the admin's own device |
| 2026-09-16 | `POST /library/roots` now rejects (and un-registers) a folder where the scan finds zero audio files, reusing the scan that already runs rather than a second walk. Verifying this against a real drive (`/mnt/d`) surfaced a separate, more serious bug found the same way as the row above — not by reading, by testing against real hardware: `findAudioFiles`'s single `readdir(root, { recursive: true })` throws on the first subdirectory it can't read, and every Windows drive has one (`System Volume Information`, `$RECYCLE.BIN`) — so any real external drive would have crashed the scan outright, not an edge case. Rewritten as a hand-rolled walk that skips one unreadable subdirectory instead of aborting the whole tree; `ScanSummary` gained `unreadableDirs`. Confirmed against the running container: registering the actual `/mnt/d` ran for several minutes without crashing or blocking other requests. 294/294 backend tests (3 new). | A directory being readable never meant it contained music; and mounting real drives (the change just above) turned a theoretical `readdir` edge case into the default case |
