...
| 4. Walkthrough | Build-verify, connect to a real backend, restart persistence | **Connect + login confirmed on-device 2026-09-18; restart persistence and error-state paths still unverified** | [Phase 4](#phase-4-walkthrough) |
...
- [ ] **Not yet confirmed:** restart persistence (Phase 0's actual done-when — "credentials persist across app restarts" — needs killing and reopening the app, not just a successful first login), and the three error paths (wrong password → 401 message, unreachable host, malformed URL). The happy path working doesn't imply these do; `mapError`'s branches are still unexercised on a real device.
