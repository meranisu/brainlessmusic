...
| 4. Walkthrough | Build-verify, connect to a real backend, restart persistence | **Build verified 2026-09-18; on-device connection test pending the owner** | [Phase 4](#phase-4-walkthrough) |
...
- [ ] **Still not run anywhere.** No emulator (no GPU/KVM passthrough into WSL2) and no device attached to this environment — the owner is installing the built APK on a physical phone (POCO F5) themselves and will test the connect → log in flow directly against the tunnel URL above.
- [ ] Once tested on-device: confirm test-connection success, login success, a wrong password shows the 401 message, an unreachable host shows the unreachable message, a malformed URL is caught client-side, and app-restart keeps the session and lands on Home. Update this checklist and `.docs/STATUS.md` with the real result.
