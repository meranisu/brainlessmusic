...
| Layer | Status | Detail |
|---|---|---|
| Backend | ✅ | Byte-range streaming (206/416, verified byte-identical), on-the-fly ffmpeg transcode via `?quality=low`, short-lived media-scoped `?token=` so an `<audio>`/`<img>` element can point straight at a URL, and waveform peaks decoded once and cached on the row |
| Web | ✅ | Real queue, native seeking, transport, repeat, shuffle and keyboard control. Below `md` the bar opens a full-screen Now Playing view with a waveform scrubber and a jump-to-track queue. No gapless |
| Mobile | ⬜ | |
